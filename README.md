...
<p>
  <img alt="Release v0.2.0" src="https://img.shields.io/badge/RELEASE-v0.2.0-e6b8b8?labelColor=2f2d42&style=flat-square">
...
- `(admin)`: route group folder ignored in the URL.
...
dynamic folders such as `[slug]`. Route group folders such as `(admin)` organize
templates and layouts without adding URL segments. Component folders are normal
Jinja template organization and do not create routes.
...
/dashboard                -> templates/(admin)/dashboard/page.html
```

Route group folders use parentheses and are ignored during automatic template
resolution:
...
The route above resolves to `templates/(admin)/dashboard/page.html` when there
is no direct `templates/dashboard/page.html`. If both exist, the direct template
wins. If multiple grouped templates match the same URL, pass `template=...`
...
components are standard Jinja macros organized by convention. Component files
can also colocate browser behavior with `{% app_script %}` blocks.
...
### Colocated Component Scripts

Use `{% app_script "unique-key" %}` in any rendered template or imported macro
file to register component JavaScript beside the component markup:

```jinja
{# templates/components/ui/tabs.html #}

{% macro root(value="", class="", attrs={}) %}
  <div
    class="{{ class }}"
    data-ui="tabs"
    data-value="{{ value }}"
...
function initTabs(root) {
  if (root.dataset.appTabsReady === "true") return;
  root.dataset.appTabsReady = "true";

  root.addEventListener("click", (event) => {
    const trigger = event.target.closest("[data-ui='tab-trigger']");
    if (!trigger || !root.contains(trigger)) return;
    root.dataset.value = trigger.dataset.value;
  });
}

document.querySelectorAll("[data-ui='tabs']").forEach(initTabs);
window.addEventListener("app-router:patch", () => {
  document.querySelectorAll("[data-ui='tabs']").forEach(initTabs);
});
{% end_app_script %}
```
...
- Scripts are deduplicated by key per response.
- Full page responses inject collected scripts before `</body>`.
- Partial navigation responses include newly rendered scripts as JSON.
- The browser runtime executes a script key only once per document.
- The default CSP receives a per-response nonce and injected scripts use it.
- `app-router:patch` fires after a partial DOM patch.
- `app-router:navigate` fires after history/state updates.

This makes the normal pattern:

```text
Python sends initial data -> Jinja renders data-* state -> JS owns local interaction
```

For server communication, keep component interactions local by default and use
explicit API routes only for lazy data, persistence, or real mutations.
...
Version `0.2.0` is packaged with `pyproject.toml`.
...
- Colocated component scripts execute once per document. Write idempotent
  initializers and listen for `app-router:patch` when components need to scan
  newly patched DOM.
...
- No duplicate route-group matches for the same URL unless `template=` is used.
...
- No prefetching.
- No automatic destroy lifecycle for colocated component scripts.
